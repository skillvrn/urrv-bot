                ...
                for row in table.find_all("tr")[1:]:
                    ...
                    if cells and len(cells) >= 2:
                        ...
                        if pos.startswith("UR"):
                            ...
                            positions.append({"position": pos, "data": data})
                return positions
    except:
        return []
...
async def send_alert(channel, pos_name, info, is_open=True):
    ...
    color = discord.Color.blue() if is_open else discord.Color.red()
    
    embed = discord.Embed(title=title, color=color, timestamp=datetime.datetime.now())
    
    if is_open:
        embed.add_field(name="📍 Position", value=f"```{pos_name}```", inline=False)
        embed.add_field(name="📡 Frequency", value=f"```{info['frequency']}```", inline=True)
        embed.add_field(name="👨‍✈️ Controller VID", value=f"```{info['vid']}```", inline=True)
        embed.add_field(name="⏰ Opened", value=f"<t:{int(datetime.datetime.now().timestamp())}:T>", inline=False)
    else:
        embed.description = f"Position **{pos_name}** is no longer active."
    
    # Добавляем строку в подвал
...
async def check_positions():
    ...
    # Send alerts
    for pos in opened:
        pos_data = next((p for p in positions_data if p["position"] == pos), None)
        if pos_data:
